...
class PostListFeaturedImage {
	...
	public function get_readme_data() {
		$plugin_data = $this->get_plugin_data();
		$transient_key = 'jem_plfi_readme_' . sanitize_key( str_replace( '.', '_', $plugin_data['Version'] ) );
		$readme = get_transient( $transient_key );
		if ( empty( $readme ) || ( defined( 'WP_DEBUG' ) && WP_DEBUG ) || isset( $_REQUEST['cache'] ) ) {
			...
			if ( $request ) {
				...
				if ( $readme = $readmeParser->parse_readme_contents( $request ) ) {
					set_transient( $transient_key, $readme, WEEK_IN_SECONDS );
				}
			}
		}
