...
// Based on Automattic_Readme class by Pippin Williamson
class CustomAutomatticReadme {

    function CustomAutomatticReadme() {
    ...
    function parse_readme_contents( $file_contents ) {
        ...
        if ( $_short_description[1] ) {
            $file_contents = $this->chop_string(
                                  $file_contents,
                                  $_short_description[1]
            );
        ...
        if ( !isset( $final_sections['description'] ) ) {
            $final_sections = array_merge(
                array( 'description' => $this->filter_text( $_short_description[2], true ) ),
                $final_sections
            );
            $excerpt        = true;
        }
