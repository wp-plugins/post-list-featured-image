...
class NextGenGalleryMediaLibraryAddon {
	 ...
	 *
	 */
	public function SortableNextGen() {
		$this->__construct();
	}
	...
	public function required_plugin_notice() {
		echo "<div id='message' class='error'><p>" . NGGMLA_PLUGIN_NAME . " requires " .
	...
	public function tab_frommedialibrary() {
		...
		<form id="nggmla-selected-images-form" action="" method="POST">
			...
			<p><a id="nggmla-select-images" class="button-secondary" href="#">Select Images</a> <span id="image-error"
...
public function medialibrary_js() {
	...
	<script id="medialibrary_js">
		jQuery(document).ready(function () {
			...
			jQuery('#nggmla-select-images').click(function (e) {
				e.preventDefault();
				if (custom_uploader) {
				...
				custom_uploader.on('select', function () {
					...
					images_preview += '</ul>';
					jQuery('#nggmla-selected-images').html(image_ids);
					jQuery('#nggmla-images-preview').html(images_preview);
				});

				// Check already selected images when form opens
				custom_uploader.on('open', function () {
					var selection = custom_uploader.state().get('selection');
					var ids = jQuery('#nggmla-selected-images input').map(function () {
			...
			// Ajax POST
			jQuery('#nggmla-selected-images-form').submit(function (e) {
				e.preventDefault();
				var copying = jQuery('#copying');
				var togallery = jQuery(this).find('#togallery');
				var togallery_name =  jQuery(this).find('#togallery_name');
				jQuery('#wpbody-content #screen-meta').next().remove('div.wrap');
				var data = {
					action: 'lib_to_ngg',
					nggmla_nonce: '<?php echo $ajax_nonce; ?>',
